...
def load_translation(language):
    ...
    translation_file = os.path.join(base_path, '.language', f"{language}.json")
...
class QobuzDLApp:
    ...
    def update_menu(self):
        ...
        file_menu.add_command(label=translate("Update Credentials", self.translations),
                              command=lambda: self.user_update_credentials(resolve_path))
...
class SettingsWindow:
    ...
    def save_settings(self):
        ...
        config = configparser.ConfigParser()
        config.read(self.app_instance.config_path)
