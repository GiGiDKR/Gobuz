...
class QobuzDLApp:
    ...
    def user_update_credentials(self):
        ...
        f.write(r.content)
        sleep(5)
        messagebox.showinfo("Credentials", "The credentials are up-to-date")

    def load_credentials(self):
        path_config = str(Path(__file__).resolve().with_name("config.ini"))
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Reset Credentials", command=self.load_credentials)
        file_menu.add_command(label="Update Credentials", command=self.user_update_credentials)
...
if __name__ == "__main__":
    root = ttk.Window(themename="superhero", alpha=1, size=(330, 570))
    root.position_center()
    results = [...]  # Définir vos résultats ici
    app = QobuzDLApp(root, results)
    root.position_center()
    root.mainloop()
